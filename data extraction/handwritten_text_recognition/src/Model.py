...
class Model:
    ...
    def __init__(self, charList, decoderType=DecoderType.BestPath, mustRestore=False, dump=False):
        ...
        # Store images without inferred text
        self.log_data = {}
        self.errFilesList = []

        # Calculate overall average probabilities/accuracy
        self.averages = {}
    ...
    def inferBatch(self, batch, calcProbability=False, probabilityOfGT=False, imagePath=None):
        ...
        if calcProbability:
            ...
            # Check sparse maatrix
            # print("Image:" + str(batch) + "sparse: " + str(sparse) + "\t texts: " + str(len(texts)))

            # No recognized text
            if len(sparse[0]) == 0:
                probs = [0]
                self.errFilesList.append(str(imagePath))
            else:
    ...
    """
    Save the list of failed images to a json file
    Save the overall average
    """
    def log(self, dir):
        ...
        average = total/len(self.averages)

        print("Saving logs to dump file: " + str(dir) + "/logs.json")
        self.log_data["Error"] = self.errFilesList
        self.log_data["Averages"] = self.averages
        self.log_data["Overall Average"] = average

        with open(dir + '/logs.json', 'w', encoding='utf-8') as f:
