    ...
    # infer text on input image
        ...
        for parent_dir in parent_dirs:
            ...
            for subDir in sub_dirs:
                ...
                # Track the number of processed forms
                dir_progress += 1
                print(f'Progress ({parent_dir}{"): %.2f" % (dir_progress/ len(parent_dirs) * 100)}% \t '
                      f'Processed {dir_progress}/{len(parent_dirs)} forms. ')
                print('Writing results to JSON: ' + curr_dir + '/dataDump.json')
